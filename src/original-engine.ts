  ...
  return {
              ...
              ...((n = (function (e, t) {
                ...
                let o = s.flatMap((i) =>
                    Array.from({ length: i.count }, () => ({
                      ...
                      width: (i.width[0] + n() * (i.width[1] - i.width[0])) * a,
                      curl: (n() - 0.5) * 1.2,
                      seed: n(),
                    })),
                  ),
                ...
                function u(e) {
                  ...
                  for (let a of o) {
                    ...
                    for (let o = 0; o < 14; o++) {
                      let l = o / 13,
                        c =
                          0.28 * Math.sin(0.8 * e + 17 * a.seed - 2.4 * l) * l +
                          0.05 * Math.sin(1.9 * e + 5 * a.seed - 4.5 * l) * l,
                        u = a.angle + a.curl * l + c;
                      o > 0 && ((n += Math.cos(u) * i), (s += Math.sin(u) * i));
                      let f =
                          0.5 *
                          a.width *
                          Math.pow(
    ...
    render: function (e) {
      ...
      (m.disable(m.BLEND),
        ...
        m.uniform1f(u.u("u_grainSize"), 1.2 * (m.drawingBufferWidth / a.width)),
...
export const createOriginalFishSimulation = function (
  e: number,
  t: number,
  fishCount = 4,
) {
    ...
    d = o.slice(0, Math.max(1, Math.min(4, fishCount))).map((a) => {
      ...
      for (let e = 0; e < 160; e++)
        ((n.trail[2 * e] = n.x - Math.cos(n.heading) * i * 0.02 * e),
          (n.trail[2 * e + 1] = n.y - Math.sin(n.heading) * i * 0.02 * e));
      return (p(n), m(n), n);
    }),
  ...
  function p(e) {
    ...
    let l = r >= 2 ? t[(r - 1) * 2] - t[(r - 2) * 2] : -Math.cos(e.heading),
      c =
        r >= 2 ? t[(r - 1) * 2 + 1] - t[(r - 2) * 2 + 1] : -Math.sin(e.heading),
      u = Math.hypot(l, c) || 1;
    for (l /= u, c /= u; r < 22; r++)
      ((t[2 * r] = t[(r - 1) * 2] + l * i),
  ...
  return {
    ...
    update: function (e, t) {
      for (let n of d) {
        ...
        let u = Math.cos(n.heading),
          ...
          M = z(j - _) - z(_ - (a - j)),
          N = z(j - k) - z(k - (i - j));
        ((b += M * (4 + 8 * Math.abs(M))), (w += N * (4 + 8 * Math.abs(N))));
        let E = 0;
        for (let e of d) {
          if (e === n) continue;
        ...
        let A = 0.7 + 0.45 * (0.5 + 0.5 * Math.sin(0.37 * t + 23 * n.seed)),
          F =
            n.cruise *
              A *
            ...
            n.fear * o * 3.4;
        ((n.speed +=
          (F - n.speed) * (1 - Math.exp(-e * (n.fear > 0.15 ? 7 : 1.1)))),
        ...
        ((n.beat += e * s * (0.7 + 2.2 * T)),
          ...
          (n.fold += ((n.fear > 0.25) - n.fold) * (1 - Math.exp(-(6 * e)))),
    ...
    emitRipples: function (e, t, a, i) {
      ...
      for (let [a, l, c, u] of (d.forEach((a, n) => {
        ...
        if (g > 0.05) {
          let t = 4 * f(0.26),
            r = 0.05 * Math.sin(e * s * 14 + 1.7 * n) * g * (1 - a.fold) * o;
          for (let e of [1, -1]) {
          ...
          let l =
            (o <= 1
              ? (function (e) {
                  if (e < 0) return 0;
                  let t = Math.sqrt(
                    Math.max(0, 1 - (Math.max(0.3 - e, 0) / 0.3) ** 2),
                  );
                  return (
                    (0.05 + 0.085 * (1 - r((e - 0.3) / 0.7, 0, 1) ** 1.4)) * t
                  );
                })(o) * s
              : 0.12 * s) + 10;
          if (
